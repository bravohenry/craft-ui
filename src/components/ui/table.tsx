...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
  ...
  return (
    <th
      className={cn(
        'relative px-4 py-3 text-left align-middle font-medium whitespace-nowrap [&:has([role=checkbox])]:pr-0 sticky top-0 z-10 shadow-[inset_0_-1.5px_0_var(--color-border)] bg-card',
        className
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<'td'>) {
  return (
    <td
      className={cn(
        'px-4 py-3 align-middle [&:has([role=checkbox])]:pr-0 shadow-[inset_0_-1px_0_var(--color-border)]',
        className
      )}
      {...props}
    />
