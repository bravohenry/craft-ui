  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        nav: cn(
          'absolute inset-x-0 top-1.5 flex h-[var(--cell-size)] w-full items-center justify-between gap-1',
        ...
        day_button: cn('border-0 p-0', classNames?.day_button),
 ...
 * Uses a fixed square size so selected/today styles stay circular.
 */
function CalendarDayButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        'inline-flex appearance-none items-center justify-center rounded-full border-0 bg-transparent p-0 text-sm',
        // Keep day as a centered square, avoid full-row rectangular selection.
        'size-[var(--cell-size)] shrink-0 select-none',
        'hover:bg-foreground/5 transition-colors cursor-pointer',
        // Selection state from modifiers
        modifiers.today && !modifiers.selected && 'bg-foreground/5 font-medium',
