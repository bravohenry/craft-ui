/**
 * [INPUT]: Depends on React, @tanstack/react-table, ./button, ./table, @/utils/cn.
 * [OUTPUT]: DataTable, SortableHeader and typed table exports for reusable grid rendering.
 * [POS]: components/ui enhanced layer for sortable/filterable/paginated data tables.
          ...
          <TableRow key={headerGroup.id}>
            {headerGroup.headers.map((header) => {
              ...
              return (
                <TableHead
                  key={header.id}
                  className={cn('p-0', meta?.fillWidth && 'w-full')}
                  style={{
            ...
            <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
              {row.getVisibleCells().map((cell) => {
                ...
                return (
                  <TableCell
                    key={cell.id}
                    className={cn('p-0', meta?.fillWidth && 'w-full', meta?.truncate && 'overflow-hidden')}
                    style={{
                      width: hasResized ? currentSize : undefined,
                      minWidth: minSize,
                      maxWidth: meta?.maxWidth,
...
export type { Column, ColumnDef, Row, TableInstance }
