/**
 * [INPUT]: nice-ticks, lucide-react, @/utils/cn, @/hooks/useScrollFade, @/context/PlatformContext, ../primitives/StyledDropdown, ./CodeBlock, ./TableExportDropdown, ../overlay/DataTableOverlay
 * [OUTPUT]: MarkdownDatatableBlock component, MarkdownDatatableBlockProps
 * [POS]: markdown/ — interactive sortable/groupable data table for datatable code blocks, consumed by Markdown renderer
...
function SortIcon({ dir }: { dir: SortDir }) {
  return (
    <svg aria-hidden="true" className={cn('w-3 h-3 shrink-0', dir ? 'opacity-60' : 'opacity-20')} viewBox="0 0 16 16" fill="currentColor">
      {dir === 'asc' ? (
          ...
          <StyledDropdownMenuSubContent style={{ zIndex: 401 }}>
            {parsed.columns.map((col) => {
              ...
              return (
                <StyledDropdownMenuItem
                  key={`sort-${col.key}`}
                  onSelect={(e: Event) => { e.preventDefault(); handleSort(col.key) }}
                >
